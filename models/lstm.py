...
class DeterministicConvLSTM(nn.Module):
    def __init__(self, in_channels, out_channels, hidden_channels, n_layers=2, args=None, conv_type='3d', ):
        ...
        if self.conv_type == '3d':
            ...
            #     nn.Conv3d(in_channels, nf, [args.num_past, self.filter_size, self.filter_size], 1, [0, self.padding, self.padding]),
            ...
            #     Conv3dLSTMCell_list.append(Conv3dLSTMCell(nf, nf, 3))
            #     Conv3dLSTMCell_list.append(nn.LayerNorm([nf, 1, self.width, self.width]))
            # self.lstm = nn.ModuleList(Conv3dLSTMCell_list)
    ...
    def forward(self, input, hidden=None):
        ...
        for i in range(self.n_layers):
            # print('h_in.shape, hidden[i].shape: ', h_in.shape,) # torch.Size([10, 16, 1, 128, 128])
